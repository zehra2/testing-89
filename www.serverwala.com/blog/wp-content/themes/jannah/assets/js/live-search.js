/*! Ajax Autocomplete for jQuery */ ! function(t) {
    "use strict";
    ...
    function r(e, s) {
        ...
        this.element = e, this.el = t(e), this.suggestions = [], this.badQueries = [], this.selectedIndex = -1, this.currentValue = this.element.value, this.intervalId = 0, this.cachedResponse = {}, this.onChangeInterval = null, this.onChange = null, this.isLocal = !1, this.suggestionsContainer = null, this.noSuggestionsContainer = null, this.options = t.extend({}, n, s), this.classes = {
            selected: "autocomplete-selected",
            suggestion: "autocomplete-suggestion"
        }, this.hint = null, this.hintValue = "", this.selection = null, this.initialize(), this.setOptions(s)
    }
    r.utils = e, t.Autocomplete = r, r.formatResult = function(t, s) {
        if (!s) return t.value;
        ...
        initialize: function() {
            var e, s = this,
                ...
                o = s.options;
            s.element.setAttribute("autocomplete", "off"), s.killerFn = function(e) {
                0 === t(e.target).closest(s.options.containerClass).length && (s.killSuggestions(), s.disableKillerFn())
            }, s.noSuggestionsContainer = t('<div class="autocomplete-no-suggestion"></div>').html(this.options.noSuggestionNotice).get(0), s.suggestionsContainer = t(o.containerClass), (e = s.suggestionsContainer).on("mouseover.autocomplete", i, function() {
        ...
        setOptions: function(e) {
            var s = this.options;
            t.extend(s, e), this.isLocal = t.isArray(s.lookup), this.isLocal && (s.lookup = this.verifySuggestionsFormat(s.lookup)), s.orientation = this.validateOrientation(s.orientation, "bottom")
        },
        clearCache: function() {
        ...
        fixPosition: function() {
            ...
            if (s === document.body || this.options.forceFixPosition) {
                ...
                if (this.el.closest(".pop-up-live-search").length > 0) {
                    ...
                    if (is_RTL) var u = Math.floor(l.left + this.el.outerWidth() - this.options.width + 1);
                    ...
                    else u = Math.floor(l.left + this.el.outerWidth() - this.options.width + 1);
                    h = {
                        top: l.top,
                        left: u
                    }
                ...
                if ("search-in-main-nav" == c ? jQuery("#theme-header").hasClass("main-nav-dark") ? c += " live-search-dark" : c += " live-search-light" : "search-in-top-nav" == c && (jQuery("#theme-header").hasClass("top-nav-dark") ? c += " live-search-dark" : c += " live-search-light"), e.attr("class", c), "auto" === i) {
                ...
                if (h.top += "top" === i ? -n : o, s !== document.body) {
                    var v, m = e.css("opacity");
                    this.visible || e.css("opacity", 0).show(), v = e.offsetParent().offset(), h.top -= v.top, h.left -= v.left, this.visible || e.css("opacity", m).hide()
                }
                e.css(h)
            }
        },
        ...
        onValueChange: function() {
            var e = this.options,
                s = this.el.val(),
                i = this.getQuery(s);
            this.selection && this.currentValue !== i && (this.selection = null, (e.onInvalidateSelection || t.noop).call(this.element)), clearInterval(this.onChangeInterval), this.currentValue = s, this.selectedIndex = -1, e.triggerSelectOnValidInput && this.isExactMatch(i) ? this.select(0) : i.length < e.minChars ? this.hide() : this.getSuggestions(i)
        },
        ...
        getSuggestionsLocal: function(e) {
            ...
            }, l && s.suggestions.length > l && (s.suggestions = s.suggestions.slice(0, l)), s
        },
        getSuggestions: function(e) {
            var s, i, n, o, l = this,
                a = l.options,
                r = a.serviceUrl;
            a.params[a.paramName] = e, i = a.ignoreParams ? null : a.params, !1 !== a.onSearchStart.call(l.element, a.params) && (t.isFunction(a.lookup) ? a.lookup(e, function(t) {
                l.suggestions = t.suggestions, l.suggest(), a.onSearchComplete.call(l.element, e, t.suggestions)
            }) : (l.isLocal ? s = l.getSuggestionsLocal(e) : (t.isFunction(r) && (r = r.call(l.element, e)), n = r + "?" + t.param(i || {}), s = l.cachedResponse[n]), s && t.isArray(s.suggestions) ? (l.suggestions = s.suggestions, l.suggest(), a.onSearchComplete.call(l.element, e, s.suggestions)) : l.isBadQuery(e) ? a.onSearchComplete.call(l.element, e, []) : (l.abortAjax(), o = {
                url: r,
                data: i,
                type: a.type,
                dataType: a.dataType
            }, t.extend(o, a.ajaxSettings), l.currentRequest = t.ajax(o).done(function(t) {
                var s;
                l.currentRequest = null, s = a.transformResult(t, e), l.processResponse(s, e, n), a.onSearchComplete.call(l.element, e, s.suggestions)
            }).fail(function(t, s, i) {
        ...
        hide: function() {
            var e = this.suggestionsContainer;
            t.isFunction(this.options.onHide) && this.visible && this.options.onHide.call(this.element, e), this.visible = !1, this.selectedIndex = -1, clearInterval(this.onChangeInterval), this.suggestionsContainer.hide(), this.signalHint(null)
        },
        suggest: function() {
            if (0 !== this.suggestions.length) {
                var e, s = this.options,
                    ...
                    h = s.beforeRender,
                    c = "";
                s.triggerSelectOnValidInput && this.isExactMatch(o) ? this.select(0) : (t.each(this.suggestions, function(t, s) {
                    i && (c += function(t, s) {
                        var n = t.data[i];
                        return e === n ? "" : '<div class="autocomplete-group"><strong>' + (e = n) + "</strong></div>"
                    }(s, 0)), c += '<div class="' + l + '" data-index="' + t + '">' + n(s, o) + "</div>"
                }), this.adjustContainerWidth(), u.detach(), r.html(c), t.isFunction(h) && h.call(this.element, r), this.fixPosition(), r.show(), s.autoSelectFirst && (this.selectedIndex = 0, r.scrollTop(0), r.children("." + l).first().addClass(a)), this.visible = !0, this.findBestHint())
            } else this.options.showNoSuggestionNotice ? this.noSuggestions() : this.hide()
        },
        noSuggestions: function() {
            var e = this.suggestionsContainer,
                s = t(this.noSuggestionsContainer);
            this.adjustContainerWidth(), s.detach(), e.empty(), e.append(s), this.fixPosition(), e.show(), this.visible = !0
        },
        adjustContainerWidth: function() {
            var t, e = this.options,
                s = this.suggestionsContainer;
            "auto" === e.width && (t = this.el.outerWidth() - 2, s.width(t > 0 ? t : 300))
        },
        ...
        processResponse: function(t, e, s) {
            var i = this.options;
            t.suggestions = this.verifySuggestionsFormat(t.suggestions), i.noCache || (this.cachedResponse[s] = t, i.preventBadQueries && 0 === t.suggestions.length && this.badQueries.push(e)), e === this.getQuery(this.currentValue) && (this.suggestions = t.suggestions, this.suggest())
        },
        activate: function(e) {
        ...
        moveUp: function() {
            if (-1 !== this.selectedIndex) return 0 === this.selectedIndex ? (this.suggestionsContainer.children().first().removeClass(this.classes.selected), this.selectedIndex = -1, this.el.val(this.currentValue), void this.findBestHint()) : void this.adjustScroll(this.selectedIndex - 1)
        },
        moveDown: function() {
        ...
        adjustScroll: function(e) {
            ...
            if (s) {
                var i, n, o, l = t(s).outerHeight();
                i = s.offsetTop, o = (n = this.suggestionsContainer.scrollTop()) + this.options.maxHeight - l, i < n ? this.suggestionsContainer.scrollTop(i) : i > o && this.suggestionsContainer.scrollTop(i - this.options.maxHeight + l), this.options.preserveInput || this.el.val(this.getValue(this.suggestions[e].value)), this.signalHint(null)
            }
        },
        onSelect: function(e) {
        ...
        dispose: function() {
            this.el.off(".autocomplete").removeData("autocomplete"), this.disableKillerFn(), t(window).off("resize.autocomplete", this.fixPositionCapture), this.suggestionsContainer.remove()
        }
    }, t.fn.autocomplete = t.fn.devbridgeAutocomplete = function(e, s) {
